...
def detect_bill_type(text):
    income_keywords = [
        "收入", "工资", "薪水", "奖金", "提成", "兼职", "报销", "退款",
        "返现", "红包", "转入", "到账", "收款", "收到", "赚了", "入账",
        "利息", "分红", "补贴", "津贴"
    ]
    expense_keywords = [
        "支出", "花了", "花费", "消费", "买", "付款", "支付", "转出",
        "扣款", "还款", "缴费", "充值", "吃", "打车", "购物"
    ]
...
def classify_income_category(text):
    if any(word in text for word in ["工资", "薪水", "薪资", "月薪"]):
        return {"category": "工资", "remark": text}
    if any(word in text for word in ["奖金", "提成", "绩效"]):
        return {"category": "奖金", "remark": text}
    if any(word in text for word in ["兼职", "副业", "外快"]):
        return {"category": "兼职", "remark": text}
    if any(word in text for word in ["报销", "退款", "返现"]):
        return {"category": "报销", "remark": text}
    if any(word in text for word in ["利息", "分红", "理财"]):
        return {"category": "理财", "remark": text}
    return {"category": "其他收入", "remark": text}
...
def parse_text_to_bill(text):
    ...
    # 这里以后可以直接替换成真正的大模型分类
    if bill_type == "收入":
        ai_result = classify_income_category(text)
    else:
...
def analyze_bills_with_ai(bills):
    ...
    income_bills = [item for item in bills if item.get("type") == "收入"]
    expense_bills = [item for item in bills if item.get("type", "支出") != "收入"]
    total_income = sum(float(item.get("amount", 0) or 0) for item in income_bills)
    total_expense = sum(float(item.get("amount", 0) or 0) for item in expense_bills)
